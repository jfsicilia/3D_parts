...
module support(length=80, link_radio=2.5, link_slot_height=10, thickness=2, 
               gap=2, flange_length=8, notch_radio=0.5) 
{
  ...
  copy_mirror([1,0,0]) 
  {
    ...
    translate([-length/2+flange_length/2, 
              0, 
              socket_height/2-thickness/2+2*flange_radio])
      ...
      rotate([90, 0, 0])
        difference() {
          ...
          translate([(flange_radio+10)/2, 0, 0])
            cube([flange_radio+10, flange_radio+10, 2*socket_width+10], 
                 center=true);
        }
  }
  // Base of the support.
...
//    cylinder(r=socket_width, h=socket_height, $fn=50, center=true);
    translate([0, 0, -(link_slot_height+10)/2 + link_slot_height/2])
      rotate([0, 0, -90])
        cylinder_with_notch(radio=link_radio, height=link_slot_height+10,
                            notch_radio=notch_radio);
  }
}
